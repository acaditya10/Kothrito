...
import { Map, Phone, CheckCircle, Power, Bike, Loader2, TrendingUp, Calendar, Award, Sun, Moon, Utensils, ShoppingBasket, Star, MessageSquare, MapPin, MessageCircle, Navigation, AlertOctagon } from 'lucide-react';
...
export default function RiderDashboard() {
  ...
  useEffect(() => {
    ...
    onSnapshot(query(collection(db, "orders"), where("status", "in", ["pending", "pending_rider"])), (snap) => {
  ...
  const handleAcceptJob = async (orderId: string) => {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        if (!orderDoc.exists() || !["pending", "pending_rider"].includes(orderDoc.data().status)) {
        ...
        <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl overflow-hidden border-2 border-slate-100 dark:border-slate-800">
          ...
          <div className="p-6 space-y-4 bg-slate-50 dark:bg-slate-950">
            ...
            <div className="grid grid-cols-2 gap-4">
              <a href={`tel:${activeOrder.userPhone}`} className="flex flex-col items-center justify-center bg-blue-100 dark:bg-blue-500/20 hover:bg-blue-200 border border-blue-200 dark:border-blue-500/30 p-4 rounded-3xl transition-colors active:scale-95 shadow-sm">
                <Phone size={24} className="mb-2 text-blue-600 dark:text-blue-400" />
                <span className="text-xs font-black text-blue-700 dark:text-blue-300 uppercase tracking-widest">{t.call}</span>
              </a>
              <a href={`https://wa.me/${activeOrder.userPhone.replace('+', '')}?text=Hi,%20I%20am%20your%20Kothrito%20Rider.%20I%20am%20on%20my%20way.`} target="_blank" rel="noreferrer" className="flex flex-col items-center justify-center bg-green-100 dark:bg-green-500/20 hover:bg-green-200 border border-green-200 dark:border-green-500/30 p-4 rounded-3xl transition-colors active:scale-95 shadow-sm">
                <MessageCircle size={24} className="mb-2 text-green-600 dark:text-green-400" />
                <span className="text-xs font-black text-green-700 dark:text-green-300 uppercase tracking-widest">WhatsApp</span>
              </a>
            </div>

            {/* Nav Row */}
            <div className="grid grid-cols-2 gap-4">
              <a href={`https://www.google.com/maps/dir/?api=1&destination=${activeOrder.pickup.lat},${activeOrder.pickup.lng}`} target="_blank" rel="noreferrer" className="flex flex-col items-center justify-center bg-slate-200 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 p-4 rounded-3xl transition-colors active:scale-95 shadow-sm">
                <Navigation size={20} className="mb-2 text-slate-800 dark:text-slate-200" />
                <span className="text-[10px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Nav to Pickup</span>
              </a>
              <a href={`https://www.google.com/maps/dir/?api=1&origin=${activeOrder.pickup.lat},${activeOrder.pickup.lng}&destination=${activeOrder.drop.lat},${activeOrder.drop.lng}`} target="_blank" rel="noreferrer" className="flex flex-col items-center justify-center bg-slate-200 dark:bg-slate-900 border border-slate-300 dark:border-slate-800 p-4 rounded-3xl transition-colors active:scale-95 shadow-sm">
                <Navigation size={20} className="mb-2 text-slate-800 dark:text-slate-200" />
                <span className="text-[10px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-widest">Nav to Drop</span>
              </a>
            </div>
            ...
            <button onClick={() => {
              if (confirm("Trigger Emergency SOS? This will instantly alert the administration and open a phone line.")) {
                updateDoc(doc(db, "orders", activeOrder.id), { sosInitiated: 'rider' }).then(() => {
                  window.location.href = "tel:0000000000"; // Admin Emergency Number Placeholder
                });
              }
            }} className="w-full mt-4 bg-red-100 dark:bg-red-500/20 text-red-600 dark:text-red-400 py-4 rounded-full font-black text-xs tracking-widest uppercase flex items-center justify-center gap-2 active:scale-95 transition-all outline outline-red-200 dark:outline-red-500/30">
              <AlertOctagon size={16} /> Emergency SOS / Admin connect
            </button>
          </div>
        </div>
      ) : (
