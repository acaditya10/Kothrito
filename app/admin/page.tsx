...
interface Order { id: string; status: string; userName: string; userPhone: string; serviceType: string; price?: number; pickup: any; drop: any; createdAt: any; userId?: string; sosInitiated?: string; }
...
interface RestaurantData { id: string; name: string; phone: string; paymentDetails: string; lat: number; lng: number; isOnline: boolean; }
...
export default function AdminDashboard() {
  ...
  const [activeModal, setActiveModal] = useState<"revenue" | "rides" | "fleet" | "pricing" | "users" | "feedback" | "locations" | "restaurants" | null>(null);
  const [selectedUser, setSelectedUser] = useState<UserData | null>(null);
  const [selectedRider, setSelectedRider] = useState<Rider | null>(null);
  const [ridesPage, setRidesPage] = useState(1);

  const [newPlace, setNewPlace] = useState({ name: "", lat: "", lng: "" });
  const [newRestaurant, setNewRestaurant] = useState({ name: "", phone: "", paymentDetails: "", lat: "", lng: "" });
  ...
  useEffect(() => {
    ...
    // Include the new Restaurant-specific bridging statuses
    onSnapshot(query(collection(db, "orders"), where("status", "in", ["pending", "accepted", "pending_payment", "cooking", "pending_rider"])), (snap: any) => {
    ...
    onSnapshot(collection(db, "restaurants"), (snap: any) => setRestaurants(snap.docs.map((d: any) => ({ id: d.id, ...d.data() } as RestaurantData))));
  ...
  const handleAddRestaurant = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!newRestaurant.name || !newRestaurant.lat || !newRestaurant.lng || !newRestaurant.paymentDetails) return;
    await addDoc(collection(db, "restaurants"), {
      name: newRestaurant.name,
      phone: newRestaurant.phone,
      paymentDetails: newRestaurant.paymentDetails,
      lat: parseFloat(newRestaurant.lat),
      lng: parseFloat(newRestaurant.lng),
      isOnline: true
    });
    setNewRestaurant({ name: "", phone: "", paymentDetails: "", lat: "", lng: "" });
  };
  ...
  return (
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-24 md:pb-0 mx-auto transition-colors duration-500 flex flex-col md:flex-row max-w-[1600px]">
      ...
      <aside className="bg-white dark:bg-slate-900 md:bg-transparent md:dark:bg-transparent p-6 border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-800 sticky top-0 z-40 md:h-screen md:w-[320px] shrink-0 flex flex-col shadow-sm md:shadow-none">
        ...
        <div className="grid grid-cols-3 md:grid-cols-2 gap-3 mb-6 md:mb-auto">
          ...
          <div onClick={() => setActiveModal('restaurants')} className="bg-white dark:bg-slate-800/50 hover:bg-slate-50 dark:hover:bg-slate-800 p-4 rounded-[1.5rem] md:rounded-2xl shadow-sm border border-transparent md:border-slate-100 md:dark:border-slate-800 cursor-pointer active:scale-95 transition-all flex flex-col items-start gap-3">
            <div className="bg-orange-50 dark:bg-orange-500/10 p-2.5 rounded-xl text-orange-500"><Utensils size={16} /></div>
            <div>
            ...
            <div className="fixed inset-y-0 right-0 z-[200] w-full md:w-[480px] bg-white dark:bg-slate-950 flex flex-col shadow-2xl animate-in slide-in-from-right duration-300 md:border-l dark:border-slate-800">
              <div className="p-6 border-b dark:border-slate-800 flex justify-between items-center sticky top-0 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md z-10">
                <h2 className="text-xl font-black dark:text-white uppercase tracking-wider">
                  ...
                  {activeModal === 'restaurants' && 'Manage Restaurants'}
                </h2>
              ...
              <div className="p-6 overflow-y-auto pb-32">
                ...
                {activeModal === 'restaurants' && (
                  <div className="space-y-6">
                    <form onSubmit={handleAddRestaurant} className="bg-slate-50 dark:bg-slate-900 border dark:border-slate-800 p-6 rounded-[2rem] shadow-sm">
                      <h3 className="font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest text-xs mb-4">Add Food Vendor</h3>
                      <div className="space-y-4">
                        <input value={newRestaurant.name} onChange={e => setNewRestaurant({ ...newRestaurant, name: e.target.value })} placeholder="Restaurant Name" className="w-full bg-white dark:bg-slate-950 p-4 rounded-xl border dark:border-slate-700 dark:text-white focus:border-orange-500 outline-none font-bold" />
                        <div className="grid grid-cols-2 gap-4">
                          <input value={newRestaurant.phone} onChange={e => setNewRestaurant({ ...newRestaurant, phone: e.target.value })} placeholder="Contact Number" className="w-full bg-white dark:bg-slate-950 p-4 rounded-xl border dark:border-slate-700 dark:text-white focus:border-orange-500 outline-none font-bold" />
                          <input value={newRestaurant.paymentDetails} onChange={e => setNewRestaurant({ ...newRestaurant, paymentDetails: e.target.value })} placeholder="UPI ID / Pay Details" className="w-full bg-white dark:bg-slate-950 p-4 rounded-xl border dark:border-slate-700 dark:text-white focus:border-orange-500 outline-none font-bold" />
                        </div>
                        <div className="flex gap-4">
                          <input value={newRestaurant.lat} onChange={e => setNewRestaurant({ ...newRestaurant, lat: e.target.value })} placeholder="Latitude (e.g. 23.081)" className="w-full bg-white dark:bg-slate-950 p-4 rounded-xl border dark:border-slate-700 dark:text-white focus:border-orange-500 outline-none font-bold" />
                          <input value={newRestaurant.lng} onChange={e => setNewRestaurant({ ...newRestaurant, lng: e.target.value })} placeholder="Longitude (e.g. 76.842)" className="w-full bg-white dark:bg-slate-950 p-4 rounded-xl border dark:border-slate-700 dark:text-white focus:border-orange-500 outline-none font-bold" />
                        </div>
                        <button type="submit" className="w-full bg-orange-500 text-white font-black py-4 rounded-xl shadow-lg active:scale-95 transition-transform flex justify-center items-center gap-2">
                          <Utensils size={18} /> Register Restaurant
                        </button>
                      </div>
                    </form>
                    ...
                    <div className="space-y-4">
                      {restaurants.map(r => (
                        <div key={r.id} className="bg-white dark:bg-slate-900 border dark:border-slate-800 p-5 rounded-[2rem] shadow-sm">
                          <div className="flex justify-between items-start mb-4">
                            <div>
                              <h4 className="font-black text-xl dark:text-white">{r.name}</h4>
                              <p className="text-xs font-bold text-slate-500 mt-1">{r.phone}</p>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-1 rounded-md ${r.isOnline ? 'bg-green-100 text-green-600' : 'bg-slate-100 text-slate-500'}`}>
                                {r.isOnline ? 'Online' : 'Offline'}
                              </span>
                            </div>
                          </div>

                          <div className="bg-slate-50 dark:bg-slate-950 p-3 rounded-xl mb-4 border border-slate-100 dark:border-slate-800">
                            <p className="text-[10px] font-black uppercase text-slate-400 mb-1">Payment Directed To</p>
                            <p className="text-sm font-bold text-slate-800 dark:text-slate-300">{r.paymentDetails}</p>
                          </div>

                          <div className="flex justify-between items-center border-t border-slate-100 dark:border-slate-800 pt-4 mt-2">
                            <p className="text-[10px] uppercase font-bold font-mono text-slate-400 opacity-80">{r.lat}, {r.lng}</p>
                            <div className="flex gap-2">
                              <button onClick={() => updateDoc(doc(db, "restaurants", r.id), { isOnline: !r.isOnline })} className={`py-2 px-4 rounded-xl font-bold text-xs uppercase ${r.isOnline ? 'bg-slate-100 text-slate-600 hover:bg-slate-200' : 'bg-green-100 text-green-700 hover:bg-green-200'} transition-colors`}>
                                Toggle Status
                              </button>
                              <button onClick={() => deleteDoc(doc(db, "restaurants", r.id))} className="p-2 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-colors">
                                <Trash2 size={16} />
                              </button>
                            </div>
                          </div>
      ...
      {orders.some((o: Order) => o.sosInitiated) && (
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center p-6 bg-red-600 animate-in fade-in duration-100">
          <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-red-500 to-red-900 opacity-90 animate-pulse"></div>
          <div className="relative z-10 bg-white/10 backdrop-blur-md w-full max-w-lg rounded-[3rem] p-10 shadow-2xl border border-white/20 text-center animate-in zoom-in-95 duration-200">
            <ShieldCheck size={80} className="mx-auto text-white mb-6 animate-bounce" />
            <h1 className="text-5xl font-black text-white uppercase tracking-widest mb-4">SOS Alert</h1>
            <p className="text-xl font-bold text-red-100 mb-8">An emergency has been triggered!</p>

            <div className="space-y-4 mb-8 text-left">
              {orders.filter((o: Order) => o.sosInitiated).map((o: Order) => (
                <div key={o.id} className="bg-white rounded-3xl p-6 shadow-xl">
                  <div className="flex justify-between items-start mb-4">
                    <span className="bg-red-100 text-red-600 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">
                      Initiated By: {o.sosInitiated === 'rider' ? 'RIDER' : 'STUDENT'}
                    </span>
                    <span className="font-bold text-slate-400 text-xs">ID: {o.id.slice(0, 6)}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Student</p>
                      <p className="font-black text-slate-800">{o.userName}</p>
                      <a href={`tel:${o.userPhone}`} className="text-blue-500 font-bold text-sm">📞 {o.userPhone}</a>
                    </div>
                    {(o as any).riderName && (
                      <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">Rider</p>
                        <p className="font-black text-slate-800">{(o as any).riderName}</p>
                        <a href={`tel:${(o as any).riderPhone}`} className="text-blue-500 font-bold text-sm">📞 {(o as any).riderPhone}</a>
                      </div>
                    )}
                  </div>

                  <button onClick={() => updateDoc(doc(db, "orders", o.id), { sosInitiated: null })} className="w-full mt-6 bg-slate-900 text-white font-black py-4 rounded-2xl uppercase tracking-widest text-xs active:scale-95 transition-transform">
                    Mark Acknowledged & Safe
                  </button>
                </div>
              ))}
            </div>
