...
import { LogIn, User, Bike, Utensils, ShoppingBasket, ChevronRight, Loader2, Phone, X } from 'lucide-react';
...
export default function Home() {
  ...
  return (
    <main className="flex flex-col h-[100dvh] w-full max-w-md mx-auto bg-white overflow-hidden relative">
      
      {/* 1. HEADER */}
      <header className="flex items-center justify-between px-6 h-[10%] shrink-0">
        <div className="flex items-center h-full py-3">
          <div onClick={() => window.location.reload()} className="relative h-8 w-32 cursor-pointer active:scale-95 transition-transform">
      ...
      <section className="h-[45%] mx-4 rounded-[2.5rem] bg-gradient-to-br from-[#f3934e] to-[#f8b55d] p-8 flex flex-col justify-center relative overflow-hidden shadow-2xl shadow-orange-200">
        <div className="relative z-10">
          ...
          <div className="flex gap-2">
            <button onClick={() => user ? setShowMap(true) : handleAuthAction()} className="bg-white text-orange-600 px-5 py-2.5 rounded-xl text-sm font-black shadow-lg active:scale-95 transition-all">Book Now</button>
      ...
      <section className="h-[45%] px-6 flex flex-col justify-center gap-4">
        ...
        {activeOrder && (
          <div className={`p-4 rounded-2xl animate-in slide-in-from-top duration-500 border ${
            activeOrder.status === 'accepted' ? 'bg-orange-600 text-white border-orange-400 shadow-xl' : 'bg-slate-50 text-slate-500 border-slate-100'
          }`}>
        ...
        <div className="grid grid-cols-3 gap-3">
          {services.map((s) => (
            <div key={s.id} onClick={() => user ? setShowMap(true) : handleAuthAction()} className="bg-slate-50 border border-slate-100 rounded-3xl p-4 flex flex-col items-center justify-center text-center gap-2 active:bg-orange-50 active:border-orange-200 transition-colors cursor-pointer">
              <div className={`${s.color} p-3 rounded-2xl shadow-sm`}>{s.icon}</div>
              <div><h4 className="text-[11px] font-black text-slate-800 leading-none">{s.title}</h4><span className="text-[9px] font-bold text-slate-400">{s.price}</span></div>
            </div>
          ))}
        </div>
